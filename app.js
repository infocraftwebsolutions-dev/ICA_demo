...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* HERO + CTA CANVAS */
  initHeroParticles(prefersReduced);
  initCtaParticles(prefersReduced);

  /* UNIVERSAL 3D TILT */
  initTilt($$('.course-card,.usp-card,.journey-step,.testimonial-card,.curr-box,.stat-card'), prefersReduced);

  /* MAGNETIC BUTTONS */
  initMagneticButtons($$('.btn-primary,.btn-ghost,.nav-cta,.mobile-cta'), prefersReduced);
...
function initTilt(elements, reduced) {
...
function initMagneticButtons(buttons, reduced) {
  if (reduced || !window.matchMedia('(pointer:fine)').matches) return;
  buttons.forEach(btn => {
...
function initHeroParticles(reduced) {
  const canvas = document.getElementById('heroCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  let W = 0, H = 0, particles = [], mouse = { x: -9999, y: -9999 };
  const COUNT = reduced ? 28 : 110;
  const CONNECT = 145;
  ...
  function tick() {
    ...
    const gradient = ctx.createRadialGradient(W * .78, H * .36, 0, W * .78, H * .36, W * .45);
...
function initCtaParticles(reduced) {
  const canvas = document.getElementById('ctaCanvas');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  let W = 0, H = 0;
  const dots = [];
  const COUNT = reduced ? 20 : 70;
  function resize() {
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
